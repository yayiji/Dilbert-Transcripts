 ...
 *   2. Comic images in: dilbert-comics/YYYY/YYYY-MM-DD.gif
 *   3. Transcripts saved to: dilbert-transcripts/YYYY/YYYY-MM-DD.json
...
function getComicFiles(year) {
    const comicsDir = path.join(__dirname, 'dilbert-comics', year.toString());
...
function transcriptExists(date) {
    ...
    const transcriptPath = path.join(__dirname, 'dilbert-transcripts', year, `${date}.json`);
...
function saveTranscript(date, transcript) {
    const year = date.split('-')[0];
    const transcriptsDir = path.join(__dirname, 'dilbert-transcripts', year);
...
async function transcribeMultipleYears(startYear, endYear) {
    ...
    for (let year = startYear; year <= endYear; year++) {
        ...
        const yearDir = path.join(__dirname, 'dilbert-comics', year.toString());
